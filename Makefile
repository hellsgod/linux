...
KBUILD_USERLDFLAGS := \
  ...
  -plugin-opt=enable-newgvn -plugin-opt=gvn-add-phi-translation -plugin-opt=enable-gvn-sink \
  -plugin-opt=enable-memcpy-dag-opt -plugin-opt=enable-heap-to-stack-conversion \
  ...
  -plugin-opt=enable-load-in-loop-pre -plugin-opt=enable-split-backedge-in-load-pre \
  ...
  -plugin-opt=lsr-drop-solution -plugin-opt=lsr-exp-narrow -plugin-opt=lsr-insns-cost \
  -plugin-opt=hot-cold-static-analysis -plugin-opt=hoist-common-insts \
  -plugin-opt=branch-hint-probability-threshold=75 -plugin-opt=enable-branch-hint -plugin-opt=hints-allow-reordering \
