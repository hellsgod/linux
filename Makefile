...
KBUILD_USERLDFLAGS := \
  ...
  -plugin-opt=enable-vtable-profile-use \
  -plugin-opt=enable-unroll-and-jam \
  -plugin-opt=enable-loop-header-duplication \
  -plugin-opt=enable-split-loopiv-heuristic \
  -plugin-opt=lsr-drop-solution \
  -plugin-opt=enable-subreg-liveness \
  -plugin-opt=enable-linkonceodr-outlining \
  -plugin-opt=enable-phi-of-ops \
  -plugin-opt=lsr-exp-narrow \
  -plugin-opt=lsr-insns-cost \
